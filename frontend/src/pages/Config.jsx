...
const EMPTY_CATEGORY = { name: '', icon: '', color_hex: '#6366f1' }
...
export function CategoriesPage() {
    ...
    function startEdit(cat) {
        setEditingId(cat.id)
        setEditForm({ name: cat.name, icon: cat.icon, color_hex: cat.color_hex })
        setAdding(false)
    ...
    async function handleDelete(cat) {
        ...
        try {
            ...
            setMsg({ type: 'error', text: 'No se puede eliminar (puede estar en uso)' })
        } finally {
            setLoading(false)
        }
    }
    ...
    async function handleAdd() {
        ...
        try {
            ...
            setMsg({ type: 'error', text: 'Error al crear categoría' })
        } finally {
            setLoading(false)
        }
    }
...
export function CotizacionPage() {
    ...
    useEffect(() => {
        ...
        setUsdToArs(existing ? existing.usd_to_ars.toString() : '')
        setNotes('')
        setMessage(null)
    }, [month, year, rates])
    ...
    async function handleSaveRate() {
        ...
        setMessage(null)
        try {
            await api.createExchangeRate({ month, year, usd_to_ars: parseFloat(usdToArs), notes: notes || null })
